...
#
# getHoldingsIds.cgi script fetches the holdings ids related to the given bibliographic record.
#
...
my $config_file = $base_path . 'get_holdings_ids.conf';
...
writelog('info', 'Starting getHoldingsIds.cgi');
...
my $response = get_ids($request_parameters{'bibId'});
...
sub get_ids($) {
  ...
  open_db_connection();
  my @ids = get_holdings_ids($bib_id);
  close_db_connection();
  ...
  for my $id (@ids) {
    $response_str .= "  <holdingsId>$id</holdingsId>\n";
  }
  $response_str .= get_xml_footer();
  if(@ids == 0) {
    $response_str =~ s/<holdingsIds>\n<\/holdingsIds>/<holdingsIds \/>/;
  }
...
sub get_holdings_ids($) {
  ...
  writelog('info', "Fetching holdings ids related to the given bib id from DB. Bib id: $bib_id");
...
sub check_bib_id() {
  ...
  if(exists($request_parameters{'bibId'})) {
    writelog('debug', 'Bib id found');
    if($request_parameters{'bibId'} =~ m/^[0-9]+$/) {
      writelog('debug', 'Bib id contains only numbers');
      return 1;
    }
    writelog('info', "Bad formed bib id: $request_parameters{'bibId'}");
    return 0;
  }
  writelog('info', 'No bib id found');
  return 0;
}
####################################
sub writelog($$)
